...
class OVNClient(object):
                ...
                for ip in ip_subnets:
                    ...
                    if (self._is_virtual_port_supported() and
                        ...
                        if parents:
                            ...
                            options[
                                ovn_const.LSP_OPTIONS_VIRTUAL_IP_KEY
                            ] = ip_addr
                            options[
    ...
    def delete_port(self, port_id, port_object=None):
        try:
            ...
            rev_row = db_rev.get_revision_row(port_id)
    ...
    def _get_nets_and_ipv6_ra_confs_for_router_port(self, port):
    ...
    def _gen_router_port_options(self, port, network=None):
        ...
        if network.get(pnet.NETWORK_TYPE) == const.TYPE_VLAN:
            options[ovn_const.LRP_OPTIONS_RESIDE_REDIR_CH] = (
                'false' if config.is_ovn_distributed_floating_ip()
                else 'true')
    ...
    def _create_lrouter_port(self, router, port, txn=None):
        ...
        networks, ipv6_ra_configs = (
            self._get_nets_and_ipv6_ra_confs_for_router_port(port))
