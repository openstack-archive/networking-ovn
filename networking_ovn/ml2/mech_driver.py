...
from networking_ovn.agent import neutron_agent as n_agent
...
class OVNMechanismDriver(api.MechanismDriver):
    ...
    def _update_dnat_entry_if_needed(self, port_id, up=True):
        ...
        if up and config.is_ovn_distributed_floating_ip():
    ...
    def delete_mac_binding_entries(self, external_ip):
        ...
        if config.get_ovn_sb_private_key():
            cmd += ['-p', config.get_ovn_sb_private_key(), '-c',
                    config.get_ovn_sb_certificate(), '-C',
                    config.get_ovn_sb_ca_cert()]
